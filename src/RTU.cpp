...
{
    int duration_us =
        ceil(1000000.0 / static_cast<double>(bitrate) * SERIAL_BITS_PER_CHAR * 3.5);
    return base::Time::fromMicroseconds(max(duration_us, 1750));
}

uint8_t* RTU::formatFrame(uint8_t* buffer,
    int address,
    int functionID,
...
static void validateBufferSize(uint8_t const* start,
    uint8_t const* end,
    char const* context)
{
    if (end - start < RTU::FRAME_OVERHEAD_SIZE) {
        throw RTU::TooSmall(string(context) +
                            ": "
                            "expected at least " +
                            to_string(RTU::FRAME_OVERHEAD_SIZE) +
                            " bytes, "
                            "but got " +
                            to_string(end - start));
    }
...
uint8_t* RTU::formatReadRegisters(uint8_t* buffer,
    uint8_t address,
...
uint8_t* RTU::formatReadDigitalInputs(uint8_t* buffer,
    uint8_t address,
    bool coils,
...
uint8_t* RTU::formatWriteRegister(uint8_t* buffer,
    uint8_t address,
    uint16_t register_id,
    uint16_t value)
{
...
uint8_t* RTU::formatWriteSingleCoil(uint8_t* buffer,
    uint8_t address,
    uint16_t register_id,
    bool value)
{
    ...
    payload[3] = 0;
    return formatFrame(buffer, address, FUNCTION_WRITE_SINGLE_COIL, payload, payload + 4);
}