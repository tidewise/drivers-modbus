...
namespace modbus {
    ...
    public:
        ...
        virtual Frame const& request(int address,
            int function,
            std::vector<uint8_t> const& payload) = 0;

        /** Read a set of registers */
        virtual std::vector<uint16_t> readRegisters(int address,
            bool input_registers,
            int start,
            int length) = 0;

        /** Read a set of registers */
        virtual void readRegisters(uint16_t* values,
            int address,
            bool input_registers,
            int start,
            int length) = 0;

        virtual uint16_t readSingleRegister(int address,
            bool input_registers,
            int register_id) = 0;

        virtual void writeSingleRegister(int address,
            uint16_t register_id,
            uint16_t value) = 0;

        virtual void writeSingleCoil(int address, uint16_t register_id, bool value) = 0;

        virtual std::vector<bool> readDigitalInputs(int address,
            bool coils,
            uint16_t register_id,
            uint16_t count) = 0;
    };
