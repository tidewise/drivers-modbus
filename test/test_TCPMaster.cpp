...
struct TCPMasterTest : public ::testing::Test,
                       iodrivers_base::Fixture<PreconfiguredDriver> {
    int pipeTX = -1;
...
TEST_F(TCPMasterTest, it_throws_if_receiving_an_unexpected_function_code_in_reply)
{
    ...
    ASSERT_THROW(driver.request(0x10, 0x02, vector<uint8_t>{1, 2, 3, 4, 5}),
        UnexpectedReply);
}
...
TEST_F(TCPMasterTest, it_does_a_holding_register_read_request)
{
    ...
    IODRIVERS_BASE_MOCK();
    EXPECT_REPLY(vector<uint8_t>{0xaa, 0x01, 0, 0, 0, 6, 0x10, 0x03, 0xab, 0xcd, 0, 2},
...
TEST_F(TCPMasterTest, it_does_a_single_holding_register_read_request)
{
    ...
    IODRIVERS_BASE_MOCK();
    EXPECT_REPLY(vector<uint8_t>{0xaa, 0x01, 0, 0, 0, 6, 0x10, 0x03, 0xab, 0xcd, 0, 1},
