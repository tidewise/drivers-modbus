...
TEST_F(TCPTest, it_formats_a_frame)
{
    ...
    uint8_t* bufferEnd =
        TCP::formatFrame(buffer, 0xabcd, 0x2, 0x10, payload, payload + 5);
...
TEST_F(TCPTest, it_handles_frames_up_to_the_maximum_allowed_by_the_frame_length)
{
    ...
    buffer.resize(65541);
    uint8_t* bufferEnd = TCP::formatFrame(&buffer[0],
        0x1234,
        0x5,
        0xa,
        &payload[0],
...
TEST_F(TCPTest,
    it_throws_if_the_resulting_frame_payload_cannot_be_represented_in_the_lenght_field)
{
    uint8_t buffer[0];
...
TEST_F(TCPTest,
    it_throws_if_attempting_to_parse_a_buffer_that_does_not_contain_the_advertised_payload)
{
    uint8_t bytes[8];
    bytes[0] = 0;
    bytes[1] = 0;
...
TEST_F(TCPTest, it_formats_a_read_holding_registers_command)
{
    ...
    uint8_t expected[EXPECTED_SIZE] =
        {0xab, 0xcd, 0, 0, 0, 6, 0x10, 3, 0x10, 0x20, 0x0, 0x05};
...
TEST_F(TCPTest, it_formats_a_read_input_registers_command)
{
    ...
    uint8_t expected[EXPECTED_SIZE] =
        {0xab, 0xcd, 0, 0, 0, 6, 0x10, 4, 0x10, 0x20, 0x0, 0x05};
    ASSERT_THAT(std::vector<uint8_t>(buffer, end), ElementsAreArray(expected));
}
...
TEST_F(TCPTest, it_formats_a_single_register_write)
{
    ...
    uint8_t expected[EXPECTED_SIZE] =
        {0xab, 0xcd, 0, 0, 0, 6, 0x10, 6, 0x10, 0x20, 0x11, 0x21};
    ASSERT_THAT(std::vector<uint8_t>(buffer, end), ElementsAreArray(expected));
}
